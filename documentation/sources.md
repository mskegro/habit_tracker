# Sources & References

## Flutter & Dart Documentation

- Flutter documentation - https://flutter.dev/docs
- Flutter API reference - https://api.flutter.dev/index.html
- Dart language guide - https://dart.dev/guides
- Flutter widget catalog - https://docs.flutter.dev/ui/widgets
- Flutter cookbook - https://docs.flutter.dev/cookbook
- Dart async/await - https://dart.dev/codelabs/async-await
- Flutter layout tutorial - https://docs.flutter.dev/ui/layout/tutorial
- Flutter getting started codelab - https://docs.flutter.dev/get-started/codelab
- Dart language tour - https://dart.dev/language
- Flutter state management - https://docs.flutter.dev/data-and-backend/state-mgmt/options
- Flutter app architecture - https://docs.flutter.dev/app-architecture
- Flutter error handling - https://docs.flutter.dev/testing/errors
- Flutter navigation basics - https://docs.flutter.dev/cookbook/navigation/navigation-basics
- Flutter named routes - https://docs.flutter.dev/cookbook/navigation/named-routes
- Flutter passing arguments - https://docs.flutter.dev/cookbook/navigation/navigate-with-arguments
- Flutter file I/O - https://docs.flutter.dev/cookbook/persistence/reading-writing-files
- Flutter key-value storage - https://docs.flutter.dev/cookbook/persistence/key-value
- Flutter internationalization - https://docs.flutter.dev/development/accessibility-and-localization/internationalization
- Flutter macOS deployment - https://docs.flutter.dev/deployment/macos
- Flutter Android deployment - https://docs.flutter.dev/deployment/android

---

## Firebase Documentation

- Firebase Flutter setup - https://firebase.google.com/docs/flutter/setup
- Firebase Firestore quickstart - https://firebase.google.com/docs/firestore/quickstart
- Firebase Auth Flutter - https://firebase.google.com/docs/auth/flutter/start
- Firebase Auth email/password - https://firebase.google.com/docs/auth/flutter/password-auth
- Firebase Auth Google Sign In - https://firebase.google.com/docs/auth/flutter/federated-auth
- Firebase manage users - https://firebase.google.com/docs/auth/flutter/manage-users
- Firebase add data - https://firebase.google.com/docs/firestore/manage-data/add-data
- Firebase update data - https://firebase.google.com/docs/firestore/manage-data/add-data#update-data
- Firebase query data - https://firebase.google.com/docs/firestore/query-data/queries
- Firebase listen real-time - https://firebase.google.com/docs/firestore/query-data/listen
- Firebase data model - https://firebase.google.com/docs/firestore/data-model
- Firebase Flutter codelab - https://firebase.google.com/codelabs/firebase-get-to-know-flutter

---

## Packages (pub.dev)

- firebase_core - https://pub.dev/packages/firebase_core
- firebase_auth - https://pub.dev/packages/firebase_auth
- cloud_firestore - https://pub.dev/packages/cloud_firestore
- firebase_messaging - https://pub.dev/packages/firebase_messaging
- google_sign_in - https://pub.dev/packages/google_sign_in
- google_sign_in usage - https://pub.dev/packages/google_sign_in#usage
- provider - https://pub.dev/packages/provider
- provider tutorial - https://pub.dev/packages/provider#getting-started
- shared_preferences - https://pub.dev/packages/shared_preferences
- flutter_local_notifications - https://pub.dev/packages/flutter_local_notifications
- timezone - https://pub.dev/packages/timezone
- fl_chart - https://pub.dev/packages/fl_chart
- lottie - https://pub.dev/packages/lottie
- confetti - https://pub.dev/packages/confetti
- flutter_colorpicker - https://pub.dev/packages/flutter_colorpicker
- intl - https://pub.dev/packages/intl
- uuid - https://pub.dev/packages/uuid
- path_provider - https://pub.dev/packages/path_provider
- cupertino_icons - https://pub.dev/packages/cupertino_icons
- flutter_lints - https://pub.dev/packages/flutter_lints
- flutter_launcher_icons - https://pub.dev/packages/flutter_launcher_icons
- flutter_launcher_icons usage - https://pub.dev/packages/flutter_launcher_icons#usage

---

## fl_chart Documentation

- fl_chart bar chart docs - https://github.com/imaNNeo/fl_chart/blob/main/repo_files/documentations/bar_chart.md
- fl_chart line chart docs - https://github.com/imaNNeo/fl_chart/blob/main/repo_files/documentations/line_chart.md
- BarChartGroupData - https://pub.dev/documentation/fl_chart/latest/fl_chart/BarChartGroupData-class.html
- BarChartRodData - https://pub.dev/documentation/fl_chart/latest/fl_chart/BarChartRodData-class.html
- LineChartBarData - https://pub.dev/documentation/fl_chart/latest/fl_chart/LineChartBarData-class.html
- FlSpot - https://pub.dev/documentation/fl_chart/latest/fl_chart/FlSpot-class.html
- FlDotCirclePainter - https://pub.dev/documentation/fl_chart/latest/fl_chart/FlDotCirclePainter-class.html

---

## Flutter Widget

- DateFormat - https://api.flutter.dev/flutter/intl/DateFormat-class.html
- SafeArea - https://api.flutter.dev/flutter/widgets/SafeArea-class.html
- SingleChildScrollView - https://api.flutter.dev/flutter/widgets/SingleChildScrollView-class.html
- LinearProgressIndicator - https://api.flutter.dev/flutter/material/LinearProgressIndicator-class.html
- ClipRRect - https://api.flutter.dev/flutter/widgets/ClipRRect-class.html
- GestureDetector - https://api.flutter.dev/flutter/widgets/GestureDetector-class.html
- InkWell - https://api.flutter.dev/flutter/material/InkWell-class.html
- IconButton - https://api.flutter.dev/flutter/material/IconButton-class.html
- DropdownButton - https://api.flutter.dev/flutter/material/DropdownButton-class.html
- AlertDialog - https://api.flutter.dev/flutter/material/AlertDialog-class.html
- SnackBar - https://api.flutter.dev/flutter/material/SnackBar-class.html
- Switch - https://api.flutter.dev/flutter/material/Switch-class.html
- CircularProgressIndicator - https://api.flutter.dev/flutter/material/CircularProgressIndicator-class.html
- GridView - https://api.flutter.dev/flutter/widgets/GridView-class.html
- SliverGridDelegateWithFixedCrossAxisCount - https://api.flutter.dev/flutter/rendering/SliverGridDelegateWithFixedCrossAxisCount-class.html
- TextOverflow - https://api.flutter.dev/flutter/painting/TextOverflow.html
- BoxDecoration - https://api.flutter.dev/flutter/painting/BoxDecoration-class.html
- LinearGradient - https://api.flutter.dev/flutter/painting/LinearGradient-class.html
- BorderRadius - https://api.flutter.dev/flutter/painting/BorderRadius-class.html
- BoxShadow - https://api.flutter.dev/flutter/painting/BoxShadow-class.html
- EdgeInsets - https://api.flutter.dev/flutter/painting/EdgeInsets-class.html
- Expanded - https://api.flutter.dev/flutter/widgets/Expanded-class.html
- Flexible - https://api.flutter.dev/flutter/widgets/Flexible-class.html
- StreamBuilder - https://api.flutter.dev/flutter/widgets/StreamBuilder-class.html
- FutureBuilder - https://api.flutter.dev/flutter/widgets/FutureBuilder-class.html
- ChangeNotifier - https://api.flutter.dev/flutter/foundation/ChangeNotifier-class.html
- ChangeNotifier tutorial - https://docs.flutter.dev/data-and-backend/state-mgmt/simple#changenotifier

---

## Dart Core Libraries

- File class - https://api.dart.dev/stable/dart-io/File-class.html
- StringBuffer - https://api.dart.dev/stable/dart-core/StringBuffer-class.html
- DateTime - https://api.dart.dev/stable/dart-core/DateTime-class.html
- Duration - https://api.dart.dev/stable/dart-core/Duration-class.html
- List - https://api.dart.dev/stable/dart-core/List-class.html
- Map - https://api.dart.dev/stable/dart-core/Map-class.html

---

## Additional Resources

- Lottie free animations - https://lottiefiles.com
- Clean architecture Flutter - https://resocoder.com/flutter-clean-architecture-tdd
- Privacy Policy template - https://www.termsfeed.com/public/uploads/2021/12/sample-mobile-app-privacy-policy-template.pdf
- MarkDown Format guide- https://www.markdownguide.org/basic-syntax/
- Gamification - https://pmc.ncbi.nlm.nih.gov/articles/PMC10998180/ 

---

## YouTube Tutorials

- Flutter Firebase setup tutorial - https://www.youtube.com/watch?v=7vLbtDMfrno
- Flutter Firebase authentication tutorial - https://www.youtube.com/watch?v=4vKiJZNPhss
- Flutter Provider state management tutorial - https://www.youtube.com/watch?v=CvRkyxJn2Fc
- Flutter & Firebase app build - https://www.youtube.com/playlist?list=PL4cUxeGkcC9j--TKIdkb3ISfRbJeJYQwC
- Flutter Firebase tutorial playlist - https://www.youtube.com/playlist?list=PLjVLYmrlmjGeA6_i1WOallrMbTzZtBcp8
- Flutter & Firebase beginner to pro 2024 (8 real projects) - https://www.youtube.com/watch?v=v0eHD-PD88g
- Flutter & Firebase full course 2024 (7 real projects) - https://youtu.be/6c3IH-DFODw?si=LbS_jxAhyIBxTuGZ
